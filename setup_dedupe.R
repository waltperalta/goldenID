...
# ESTABLISH CONNECTION
  ...
  # OSC: SELECT memno,email,phone,fname,lname,zip,country,yyyy,mm,dd,address FROM tmp.SM_OSCcustomer_cleansed
  # WP_goldenid_dedupe_OSC_28022019

  # IDM: SELECT memno,email,phone,fname,lname,zip,country,yyyy,mm,dd,address FROM tmp.IDM_cleansed WHERE memno IS NOT NULL
  # WP_goldenid_dedupe_IDM_28022019
...
####################################
colnames(output)
output_deduped <- dedupe(output, output$email, exclude = c(1:2), blockfld = c(10:11))
